document.addEventListener("DOMContentLoaded", () => {
  ...
  if (linkDisplay) {
    const linkStates = [
      "MONITORING CITY NETWORK",
      "CALCULATING TRUST SCORE",
      "VERIFYING CONSENT TOKENS",
      "AUDITING AI FAIRNESS",
      "SCANNING BIAS ANOMALIES",
      "REBALANCING PRIVACY INDEX",
      "PHANTOMPASS VERIFIED",
      "CIVICVAULT SECURED",
      "CONSENTCAM ACTIVE",
      "FAIRWATCH MONITORING",
      "ETHICS AUDIT PASSED",
      "SURVEILLANCE THROTTLED",
      "CITIZEN PRIVACY PROTECTED",
      "MULTI-PARTY CONSENSUS READY",
      "ZERO-KNOWLEDGE PROOFS VALID",
      "TRUST SCORE STABLE (94)",
      "SYNCING CITY NODES",
      "ENCRYPTING CITIZEN CHANNELS",
      "VALIDATING ACCESS POLICIES",
      "MONITORING CONSENT REGISTRY",
      "UPDATING FAIRNESS METRICS",
      "CHECKING PRIVACY GUARDRAILS",
      "ANALYZING SECURITY EVENTS",
      "VERIFYING DIGITAL CREDENTIALS",
      "PROCESSING THREAT SIGNALS",
      "SCANNING ANOMALOUS BEHAVIOR",
      "TRUST ENGINE ONLINE",
      "ETHICAL GOVERNANCE ACTIVE",
      "CONSENSUS NETWORK HEALTHY",
      "CITY STATUS BALANCED",
      "PRIVACY-SAFETY EQUILIBRIUM MAINTAINED",
      "CONSENT REGISTRY SYNCHRONIZED",
      "BIAS THRESHOLD WITHIN LIMITS",
      "SECURE CHANNELS OPERATIONAL",
      "SURVEILLANCE LIMITS ENFORCED",
      "CREDENTIAL CACHE VERIFIED",
      "ANONYMIZATION PIPELINE ACTIVE",
      "PUBLIC TRUST INDEX HEALTHY",
      "AUTONOMOUS COMPLIANCE CHECK COMPLETE",
      "AEGIS CORE OPERATIONAL"
    ];
    let lIdx = 0;
    setInterval(() => {
  ...
  const aegisLogs = [
    { text: "[08.21%] CONSENT_TOKEN_REGISTRY... SYNCHRONIZED", color: "text-primary-fixed" },
    { text: "[14.55%] VERIFYING_ZK_CREDENTIALS... COMPLETED", color: "text-primary-fixed" },
    { text: "[32.10%] WARNING: FAIRNESS_THRESHOLD_DRIFT (3.2%)", color: "text-tertiary-fixed" },
    { text: "[56.88%] SCANNING_CITY_ACTIVITY... 4 BIAS_ALERTS_FOUND", color: "text-secondary" },
    { text: "[78.42%] CRITICAL: TRUST_SCORE_DECREASE_DETECTED", color: "text-[#FF0000]" },
    { text: "[99.99%] PRIVACY_SAFETY_BALANCE_RESTORED", color: "text-[#00ff41]" }
  ];
          ...
          return new Promise(resolve => {
            ...
            if (progressBar) {
              ...
              setTimeout(() => {
                ...
                if (bootStatus) {
                  bootStatus.textContent = 'INITIALIZED SYSTEM BOOT...';
                  bootStatus.classList.remove('text-yellow-400');
                  bootStatus.classList.add('text-primary-fixed-dim');
                }
      ...
      const processLines = async () => {
        ...
        for (let i = 0; i < terminalLines.length; i++) {
          ...
          if (passedProgressBar && line.tagName === 'P') {
            ...
            // Respect custom coloring configured in HTML
            const hasCustomColor = Array.from(line.classList).some(cls =>
              cls.startsWith('text-green-') ||
              cls.startsWith('text-yellow-') ||
              cls.startsWith('text-red-') ||
              cls.startsWith('text-orange-') ||
